	...
	<?php if ( have_posts() ) : while( $the_query->have_posts() ) : $the_query->the_post(); ?>
		...
		<?php if (get_field('role') == "Lighting Designer") { ?>
			<div class="row subpage-indent resume-rows">
				<p class="small-4 medium-3 columns show-title"><?php the_field('show_name'); ?></p>
				<a href="<?php the_field('theatre_website'); ?>"><p class="small-4 medium-4 columns"><?php the_field('theatre'); ?></p></a>
				<p class="small-4 medium-3 columns"><?php the_field('director'); ?></p>
				<p class="small-4 medium-2 columns show-for-medium-up"><?php the_field('year'); ?></p>
			</div>		
		<?php } ?>
